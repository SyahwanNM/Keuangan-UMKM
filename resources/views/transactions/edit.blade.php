                ...
                <div class="p-6">
                    <form action="{{ route('transactions.update', $transaction) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="amount" id="amount" value="{{ old('amount', $transaction->amount) }}" required min="0" step="0.01"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      ...
                                      placeholder="Deskripsi detail transaksi...">{{ old('description', $transaction->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        @if($transaction->proof)
                            <div class="mb-4">
                                <label class="block text-sm font-medium style="color: var(--color-text_secondary, #6B7280);" mb-2">Bukti Transaksi Saat Ini</label>
                                <div class="flex items-center space-x-4">
                                    <img src="{{ Storage::url($transaction->proof) }}" alt="Bukti Transaksi" class="w-20 h-20 object-cover rounded">
                                    <div>
                                        <a href="{{ Storage::url($transaction->proof) }}" target="_blank" class="text-blue-600 hover:text-blue-800">Lihat Bukti</a>
                                        <p class="text-sm style="color: var(--color-text_secondary, #6B7280);"">Upload file baru untuk mengganti bukti ini</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                        ...
                        <!-- New Proof -->
                        <div class="mb-6">
                            <label for="proof" class="block text-sm font-medium style="color: var(--color-text_secondary, #6B7280);" mb-2">
                                {{ $transaction->proof ? 'Ganti Bukti Transaksi (Opsional)' : 'Bukti Transaksi (Opsional)' }}
                            </label>
