        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="dashboard-card overflow-hidden shadow-lg sm:rounded-xl border mb-6" style="border-color: var(--color-border, #F3F4F6);">
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('reports.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold" style="color: var(--color-primary, #8B5CF6);">Rp {{ number_format($totalCapital, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold" style="color: var(--color-success, #10B981);">Rp {{ number_format($totalIncome, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold" style="color: var(--color-danger, #EF4444);">Rp {{ number_format($totalExpense, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Laba/Rugi Bersih -->
                <div class="dashboard-card overflow-hidden shadow-lg sm:rounded-xl border-l-4 {{ $netProfit >= 0 ? 'border-indigo-500' : 'border-amber-500' }} hover:shadow-xl transition-all duration-300 {{ $netProfit >= 0 ? 'hover:border-indigo-600' : 'hover:border-amber-600' }}">
                    <div class="p-6">
                        <div class="flex items-center">
                            <div class="flex-shrink-0">
                                <div class="w-8 h-8 {{ $netProfit >= 0 ? 'bg-gradient-to-r from-indigo-100 to-blue-100' : 'bg-gradient-to-r from-amber-100 to-yellow-100' }} rounded-full flex items-center justify-center">
                                    <svg class="w-4 h-4" style="color: {{ $netProfit >= 0 ? 'var(--color-primary, #3B82F6)' : 'var(--color-warning, #F59E0B)' }};" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                                    </svg>
                                </div>
                            </div>
                            <div class="ml-4">
                                <p class="text-sm font-medium" style="color: var(--color-text_secondary, #6B7280);">{{ $netProfit >= 0 ? 'Laba' : 'Rugi' }} Bersih</p>
                                <p class="text-2xl font-semibold" style="color: {{ $netProfit >= 0 ? 'var(--color-primary, #3B82F6)' : 'var(--color-warning, #F59E0B)' }};">
                            ...
                            <div class="space-y-3">
                                @foreach($incomeByCategory as $category => $amount)
                                    <div class="flex items-center justify-between p-3 rounded-lg" style="background: var(--color-success-light, #D1FAE5);">
                                        <span class="font-bold text-lg" style="color: var(--color-success-dark, #065F46);">{{ $category }}</span>
                                        <span class="font-bold text-lg" style="color: var(--color-success-dark, #065F46);">Rp {{ number_format($amount, 0, ',', '.') }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            ...
                            <div class="space-y-3">
                                @foreach($expenseByCategory as $category => $amount)
                                    <div class="flex items-center justify-between p-3 rounded-lg" style="background: var(--color-danger-light, #FEE2E2);">
                                        <span class="font-bold text-lg" style="color: var(--color-danger-dark, #991B1B);">{{ $category }}</span>
                                        <span class="font-bold text-lg" style="color: var(--color-danger-dark, #991B1B);">Rp {{ number_format($amount, 0, ',', '.') }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @else
            ...
            @if(isset($reportData) && count($reportData['insights']) > 0)
            <div class="dashboard-card overflow-hidden shadow-lg sm:rounded-xl border mb-6" style="border-color: var(--color-border, #F3F4F6);">
                <div class="p-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold mb-4" style="color: var(--color-text, #1F2937);">Detail Transaksi ({{ $transactions->count() }} transaksi)</h3>
