...
class AutoSaveAllTaxesCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tax:auto-save-all {--year=} {--force}';
     ...
     * @var string
     */
    protected $description = 'Otomatis menyimpan semua pajak berdasarkan data transaksi untuk setiap bulan';
    ...
    {
        $this->info('🚀 Memulai penyimpanan otomatis semua pajak...');

        $year = $this->option('year') ?: Carbon::now()->year;
        $force = $this->option('force');

        $this->info("📅 Memproses pajak untuk tahun: {$year}");
        ...
        foreach ($users as $user) {
            try {
                ...
                for ($month = 1; $month <= 12; $month++) {
                    ...
                    // Cek apakah ada transaksi untuk bulan ini
                    $hasTransactions = Transaction::where('user_id', $user->id)
                        ->whereBetween('date', [$startDate, $endDate])
                        ->exists();

                    if (!$hasTransactions) {
                        $this->warn("  ℹ️  Tidak ada transaksi untuk {$user->name} pada {$startDate->format('F Y')}.");
                        continue;
                    }
                    ...
                    if (empty($taxCalculation['taxes'])) {
                        $this->warn("  ℹ️  Tidak ada pajak yang harus dibayar untuk {$user->name} pada {$startDate->format('F Y')}.");
                        continue;
                    }
                    ...
                    $this->info("  ✅ Berhasil menyimpan " . count($savedTaxes) . " pajak untuk {$user->name} pada {$startDate->format('F Y')} (Total: Rp " . number_format($userTotalTax, 0, ',', '.') . ")");

                    // Log aktivitas
                    Log::info("Auto saved taxes for user {$user->id} ({$user->name}) for {$month}/{$year}", [
                        'user_id' => $user->id,
                        'month' => $month,
                        'year' => $year,
                ...
                Log::error("Error in auto save all taxes for user {$user->id}", [
                    'user_id' => $user->id,
                    'error' => $e->getMessage(),
        ...
        // Tampilkan ringkasan
        $this->newLine();
        $this->info('📊 RINGKASAN PENYIMPANAN PAJAK OTOMATIS');
        $this->info('=====================================');
        ...
        $this->newLine();
        $this->info('✅ Penyimpanan pajak otomatis selesai!');
    ...
    private function saveAutoCalculatedTaxes($user, $taxes, $periodDate)
    {
        ...
        foreach ($taxes as $tax) {
            if ($tax['amount'] > 0) {
                ...
                $savedTax->is_auto_calculated = true;
